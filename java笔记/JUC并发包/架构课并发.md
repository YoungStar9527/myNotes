...
public abstract class AbstractQueuedSynchronizer extends AbstractOwnableSynchronizer implements java.io.Serializable 
    ...
    private Node addWaiter(Node mode) {
        ...
        Node pred = tail;
        //因为是这是两个线程竞争锁，线程2没竞争到锁，第一次尝试进入阻塞队列，所以队列目前没有节点
        //再判断一下队列上有没有节点，没有就创建一个队列
        if (pred != null) {
            node.prev = pred;
...
public abstract class AbstractQueuedSynchronizer extends AbstractOwnableSynchronizer implements java.io.Serializable 
    ...
    private Node enq(final Node node) {
        ...
        for (;;) {
            ...
            if (t == null) { 
                 //这里用了CAS设置头Node
                //当然有可能线程2设置头Node的时候CPU切换了
                //线程3  已经把头Node设置好了形成了上图所示的一个队列，这时线程2再循环一次获取tail
                if (compareAndSetHead(new Node()))
                    tail = head;
            } else {
                //由于tail是volatile的，所以对线程2可见，线程2看见tail不为null
                //去往尾Node后面添加自身。整个过程下来，形成了一个双向队列
                node.prev = t;
        ...
        try {
            ...
            for (;;) {
                final Node p = node.predecessor();
                 //此时再做判断，再来尝试获取锁
                //由于线程2是双向队列的真正的第一个Node（前面还有一个h）， 再次判断一下线程2能不能获取锁（可能这段时间内线程1已经执行完了把锁释放了，state从1变为了0）
                ...
                //如果还是没有获取到锁，先调用AQS的shouldParkAfterFailedAcquire(p, node)方法
...
​	这个waitStatus是h的waitStatus，是0的话，上面的acquireQueued的17行if自然不成立，再走一次for循环，还是先尝试获取锁，不成功，继续走shouldParkAfterFailedAcquire，此时waitStatus为-1，小于0，返回true。然后走acquireQueued的17行if的第二个判断条件parkAndCheckInterrupt
...
​	至此，使用ReentrantLock让线程1独占锁、线程2进入FIFO队列并阻塞的完整流程已经整理出来了
...
​	实际是调用sync的release方法，而Sync是继承了AQS，方法即是父类的release方法

​	unlock方法，公平锁/非公平锁是一样的
...
​	这是一个很巧妙的设计，又回到了AQS的acquireQueued方法了

```java
public abstract class AbstractQueuedSynchronizer extends AbstractOwnableSynchronizer implements java.io.Serializable 
    //被阻塞的线程2是被阻塞在第16行，注意这里并没有return语句，也就是说，阻塞完成线程2依然会进行for循环。然后，阻塞完成了，线程2所在的Node的前驱Node是p，线程2尝试tryAcquire，成功，然后线程2就成为了head节点了，把p的next设置为null，这样原头Node里面的所有对象都不指向任何块内存空间，h属于栈内存的内容，方法结束被自动回收，这样随着方法的调用完毕，原头Node也没有任何的引用指向它了，这样它就被GC自动回收了。此时，遇到一个return语句，acquireQueued方法结束，后面的Node也是一样的原理。
    final boolean acquireQueued(final Node node, int arg) {
        boolean failed = true;
        try {
            boolean interrupted = false;
